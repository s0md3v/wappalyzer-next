...
const getDetections = async (target) => {
...
;(async () => {
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    const response = await getDetections(target)

    if (response?.__error) {
      done({ error: response.__error })
...
"""
    ...
    try:
        driver.execute_async_script(script, max_duration_ms)
    except Exception:
        pass
